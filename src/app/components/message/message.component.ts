...
import { MessageService } from '../../stores/message-management-store/message.services';
...
export class MessageTableComponent implements OnInit{
  ...
  constructor(private messageService: MessageService, private toastr: ToastrService, private store:Store) {}
  scoreIds!: any[];
  id:any
  ...
  type: string = 'date';
    ...
    standardColumns: GameColumn[] = [
        ...
        { key: 'templateKey', name: 'Template Key', type: 'text' },
        { key: 'message', name: 'Message', type: 'text' },
        { key: 'imageURL', name: 'Image', type: 'image' },
        { key: 'isActive', name: 'Is Active', type: 'text' },
      ];
  ...
  updatePageCount(): void {
    this.messageService.MessagePageNumber(this.filter).subscribe(val => {
  ...
  updatePaginatedData(): void {
    ...
    this.messageService.Message(this.filter,currentPage).subscribe(val => {
