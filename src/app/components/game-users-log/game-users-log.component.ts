...
@Component({
  selector: 'app-game-users-log',
  templateUrl: './game-users-log.component.html',
  styleUrls: ['./game-users-log.component.css'], // corrected to 'styleUrls'
  standalone: false,
})
export class GameUsersLogTableComponent implements OnInit, OnChanges {
  scoreIds!: any[];
  id:any
  ...
  constructor(private gamesUsersLogService: GamesUsersLogService) {}


  paginatedData: GameUsersLogData[] = [];
  ...
  updatePageCount(): void {
    this.gamesUsersLogService.GamesUsersLogPageNumber(this.filter).subscribe(val => {
  ...
  updatePaginatedData(): void {
    ...
    this.gamesUsersLogService.GamesUsersLog(this.filter,currentPage).subscribe(val => {
