import { LikeCardsTransactionsData } from './../../stores/like-cards-transactions-management-store/like-cards-transactions.services';
...
import { LikeCardsTransactionsService } from '../../stores/like-cards-transactions-management-store/like-cards-transactions.services';

@Component({
  selector: 'app-like-card-transaction-table',
  templateUrl: './like-card-transaction-table.component.html',
  styleUrls: ['./like-card-transaction-table.component.css'], // corrected to 'styleUrls'
  standalone: false,
})
export class LikeCardTransactionTableComponent implements OnInit, OnChanges {
  ...
  filter = {
    ...
    standardColumns: GameColumn[] = [
        ...
        { key: 'deductionAmount', name: 'Deduction Amount', type: 'text'},
        { key: 'game', name: 'Game', type: 'text' },
        { key: 'transactionType', name: 'Transaction Type', type: 'text' },
        { key: 'created', name: 'Created At', type: 'date' },
      ];

  constructor(private likeCardsTransactionsService: LikeCardsTransactionsService) {}


  paginatedData: LikeCardsTransactionsData[] = [];
  ...
  updatePageCount(): void {
    this.likeCardsTransactionsService.LikeCardsTransactionsPageNumber(this.filter).subscribe(val => {
  ...
  updatePaginatedData(): void {
    ...
    this.likeCardsTransactionsService.LikeCardsTransactions(this.filter,currentPage).subscribe(val => {
